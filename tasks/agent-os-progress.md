...
- 当前状态：`[x]` C20 control-plane 实现、RED/GREEN、全量自动化验证、浏览器 QA、独立代码审查、安全复核和 scoped commit 均已完成；不宣称已接入真实外部执行器或辅助 Judge provider。
- Commit：`05c4a51baec221f83db821b83ed14fe5626cbc7a`（`feat: add goal mode loop and judge control plane`）；C19 基线为 `174c288f6365eeab879d08e6cad9f04c28029a78`。
...
- 视觉 QA：真实浏览器访问 `http://127.0.0.1:4313/goal-mode?workspace=ws-demo`；fresh rerun PNGs `c20-goal-mode-desktop-1440x900-rerun-20260901-215259.png`、`c20-goal-mode-post-judge-1440x900-rerun-20260901-215259.png`、`c20-goal-mode-mobile-390x844-rerun-20260901-215259.png`、`c20-goal-mode-mobile-controls-390x844-rerun-20260901-215259.png` 均经 `file` 验证尺寸。
...
- 下一步唯一动作：继续 C21 Skills/Learning contracts RED，保持 approved snapshot、source hash/diff/rollback 和 quarantine 边界；真实 MCP/外部执行继续禁止。

### Handoff 2026-09-02 / C21 Skills and Learning

- 当前状态：`[x]` C21 control-plane 实现、RED/GREEN、全量自动化验证、浏览器 QA、独立审查 P1 修复、本地安全复核和 scoped commit 均已完成；不宣称已执行真实 skill、MCP、URL/PDF/文件读取、凭据读取或外部安装。
- Commit：`51ab5e76a6a9a57736f03281c34e33afc297adce`（`feat: add skills learning approved snapshots`）；C20 基线为 `05c4a51baec221f83db821b83ed14fe5626cbc7a`。
- 本次完成：SkillDescriptor、SkillVersion、SkillSource、SkillScan、SkillReview、SkillInstallation、SkillInvocationFact、LearningCandidate、LearningCommand、`/learn`、approve/install/revoke/quarantine/rollback、approved snapshot trust、source hash/diff、API/CLI/Mission Control `/skills` 控制面语义。
- 持久化：SQLite migration 12 新增 skills/versions/sources/scans/reviews/installations/invocation facts/learning candidates/learning commands；scope、approved snapshot、append-only、one-active-install、proposed skill、source-event 和 rollback no-op invariants 由 repository 与 SQL triggers 双层强制。
- 自动化验证：C21 focused PASS（8 files / 75 tests）；`pnpm lint` PASS（335 source files）；`pnpm typecheck` PASS；full Vitest PASS（106 files / 584 tests）；integration PASS（14 files / 37 tests）；E2E PASS（6 tests）；Mission Control build PASS；production audit PASS。
- 视觉 QA：真实浏览器访问 `http://127.0.0.1:4313/skills?workspace=ws-demo`；fresh PNGs `c21-skills-learning-desktop-1440x900-20260902-104618.png`、`c21-skills-learning-post-actions-1440x900-20260902-104618.png`、`c21-skills-learning-mobile-390x844-20260902-104618.png`、`c21-skills-learning-invalid-workspace-390x844-20260902-104618.png` 均经 `file` 验证为 PNG，尺寸分别为 1440x900、1440x900、390x844、390x844。
- 审查结果：独立代码审查发现 P1 no-op rollback repository/SQL parity gap，已用 RED/GREEN 修复；post-fix 独立复核遇到外部模型流中断，独立安全 reviewer 因 agent thread limit 未运行，本地代码/安全清单覆盖 scope、RBAC、idempotency、optimistic concurrency、SQL 参数化/FK/triggers、append-only facts、approved snapshot、secret redaction 和 no external call，无 blocking finding。
- 安全边界：C21 只记录 descriptor、source hash、diff 摘要和 approved snapshot；拒绝 `file://`、`secret://`、`vault://providers/...`、绝对路径和 secret-shaped 文本；禁止真实 MCP 执行、真实 URL/PDF/凭据读取、外部 skill 安装、网络/provider/SaaS 副作用。
- 环境限制：主机 Node `v25.9.0` 超出项目 `<23` engine range，pnpm 仅发出 warning；首次 integration 失败源于缺少 `node_modules`，`pnpm install --frozen-lockfile` 后复跑通过；首次 E2E 失败源于缺少 Playwright Chromium，安装浏览器后复跑通过；Mission Control build 仍有第三方 `use client` bundler warning。
- 证据路径：`artifacts/progress/c21/verification.log`、`artifacts/progress/c21/current-verification/`、`docs/adr/0016-skills-learning-approved-snapshot.md`、`docs/runbooks/skills-learning-operations.md`、`packages/persistence/src/c21-skills-learning.test.ts`、`apps/api/src/routes/skills-learning.test.ts`、`apps/mission-control/src/pages/skills-learning.test.tsx`。
- 下一步唯一动作：从 C22.1 开始写 Obsidian/OMI/Journal contracts RED，保持 control-plane/descriptor-only、scope/secret scan、默认只读和写回批准边界。

### Handoff 2026-09-02 / C22 Obsidian, OMI, and Journal

- 当前状态：`[x]` C22 control-plane 实现、RED/GREEN、全量自动化验证、浏览器 QA、cross-vault raw SQL、vault policy parity、CLI/API timestamp idempotency、SQLite `LIKE` descriptor-ref、Unicode whitespace、unknown payload keys、writeback version jump、default-ignorable refs、encoded secret marker、target-root binding、safe idempotency key blocker 修复和 scoped commit 均已完成；最终 subagent 复核受平台线程上限限制，未伪造独立通过。
- Commit：`12451447dfbfcf9fd466988d99017bf896caf7c7`（`feat: add journal vault control plane`）；C21 基线为 `51ab5e76a6a9a57736f03281c34e33afc297adce`。
- 本次完成：VaultBridgeDescriptor、JournalEntryDescriptor、JournalSource、GraphIndexSnapshot、MemoryCandidate、WritebackRequest/Decision、daily journal、graph/FTS snapshot、memory candidate、writeback request/approve/reject、CLI parser、Journal API routes 和 Mission Control `/journal` 控制面语义。
- 持久化：SQLite migration 13 新增 vault/journal/source/graph/memory/writeback tables；root journal facts append-only，writeback request 仅能通过 append-only decision 从 pending 转为 approved/rejected，source/candidate/request same-workspace/same-vault invariants 由 repository 与 SQL triggers 双层强制；writeback `target_ref` 必须等于对应 vault `root_ref` 或其子路径；descriptor refs 由 case-sensitive `GLOB` checks 和 `c22_*_ref_shape_insert` triggers 拒绝 uppercase scheme、ASCII/Unicode whitespace、nested scheme、traversal 和 encoded traversal；payload exact-key triggers 拒绝未知 top-level payload fields；writeback request update trigger 要求 `NEW.version = OLD.version + 1`。
- 自动化验证：C22 focused PASS（9 files / 54 tests，`artifacts/progress/c22/current-verification/full-matrix-final-20260903-005153/c22-focused.log`）；`pnpm lint` PASS（350 source files）；`pnpm typecheck` PASS；full Vitest PASS（113 files / 630 tests）；integration PASS（14 files / 37 tests）；E2E PASS（6 tests）；Mission Control build PASS；fresh matrix summary `artifacts/progress/c22/current-verification/full-matrix-final-20260903-005153/summary.log`。
- 视觉 QA：真实浏览器访问 `http://127.0.0.1:4313/journal?workspace=ws-demo`；fresh PNGs `c22-journal-desktop-final-rerun-20260902T170407Z.png`、`c22-journal-desktop-post-actions-final-rerun-20260902T170407Z.png`、`c22-journal-mobile-final-rerun-20260902T170407Z.png` 均经 `/usr/bin/file` 验证为 PNG，尺寸分别为 1440x900、1440x900、390x844；移动端无横向溢出，五项底部导航可见，无非法 ARIA refs，无宽表格；observed 401 -> local bootstrap 204 -> scoped reads 200、writeback request 202、approve 202；earlier failed rerun traced to relative `NEXORA_DB_PATH`, final rerun used the absolute SQLite path shared by seed/API/UI。
- 审查结果：旧独立代码审查发现 HIGH cross-vault writeback candidate raw SQL gap、vault policy parity gap、CLI timestamp idempotency gap，替代代码审查发现 HIGH SQLite `LIKE` case-insensitive descriptor-ref gap，独立安全复核发现 Unicode whitespace 和 unknown payload raw-SQL gaps，最终代码审查发现 writeback version jump gap，后续安全复核发现 target-root binding 和 idempotency key hygiene gaps；default-ignorable refs 和 encoded secret markers 也已补 RED/GREEN。上述问题均已用 contracts/service/migration trigger/CHECK 和 regression 修复。最终 subagent dispatch 报告 `agent thread limit reached`，继承 agent IDs 返回 `not_found`，因此使用本地代码/安全复核清单和 fresh focused/full/visual 证据替代，不伪造独立通过。
- 安全边界：C22 默认只读 descriptor/control-plane；允许引用仅限 `workspace://`、`artifact://`、`memory://`、`journal://`；拒绝 `file://`、`secret://`、HTTP(S)、绝对路径、traversal 和 secret-shaped text；禁止连接真实 Obsidian/OMI/MCP，禁止读取真实凭据，禁止直接写用户 vault，批准写回仍只是本地 descriptor fact。
- 环境限制：主机 Node `v25.9.0` 超出项目 `<23` engine range，pnpm 仅发出 warning；Mission Control build 仍有第三方 `use client` bundler warning 和 chunk-size warning。
- 证据路径：`artifacts/progress/c22/verification.log`、`artifacts/progress/c22/current-verification/`、`docs/adr/0017-obsidian-omi-journal-control-plane.md`、`docs/runbooks/journal-operations.md`、`packages/persistence/src/c22-journal.test.ts`、`apps/api/src/routes/journal.test.ts`、`apps/mission-control/src/pages/journal.test.tsx`。
- 下一步唯一动作：进入 C23 Browser/Computer Use RED tests，先覆盖 sandbox、allowlist、screenshot/action receipt、manual approval、stop/takeover、SSRF/越权拒绝和 API/UI 控制面语义。
...
- 视觉 QA：真实浏览器访问 `http://127.0.0.1:4313/browser-computer?workspace=ws-demo`；fresh PNGs `c23-browser-computer-desktop-20260903T193322.png`、`c23-browser-computer-desktop-post-actions-20260903T193322.png`、`c23-browser-computer-mobile-20260903T193322.png` 均经 `file` 验证为 PNG，尺寸分别为 1440x900、1440x900、390x844；summary 确认无横向溢出、无非法 ARIA refs、五项移动导航、Pause/Stop/Takeover/Approve/Acknowledge 控件、session heading 可读、descriptor-only/no-external-connection 边界可见，请求来源仅 `127.0.0.1:4310` 和 `127.0.0.1:4313`。
- 审查结果：独立代码审查发现 CRITICAL dangling screenshot reference blocker，已用 RED/GREEN 修复 repository 和 raw SQL 两条路径；post-fix 独立代码复审无 blocking finding；独立安全复核无 blocking finding；本地 `git diff --check` 无 whitespace error，`pnpm audit --audit-level=high` 无已知高危漏洞。
