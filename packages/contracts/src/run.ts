...
const BudgetSchema = z.object({ max_tokens: NonNegativeInt, max_cost_usd: z.number().finite().nonnegative().max(1000000000) }).strict();
...
export const AttemptSchema = MetadataSchema.extend({ run_id: UlidSchema, status: AttemptStatusSchema }).strict();
