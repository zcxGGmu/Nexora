...
export function migrate(database: SqliteDatabase, clock: Clock = systemClock): void {
  ...
  const applied = database.prepare("SELECT version FROM schema_migrations WHERE version = ?").get(CORE_MIGRATION_VERSION);
  if (applied !== undefined) {
    validateMigration(database);
    return;
  }

  const sql = readFileSync(new URL("./migrations/0001_core.sql", import.meta.url), "utf8");
  withTransaction(database, () => {
    database.exec(sql);
    database.prepare("INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)").run(CORE_MIGRATION_VERSION, clock.now());
  });
}
...
export function rollback(database: SqliteDatabase): void {
  ...
  withTransaction(database, () => {
    database.exec(`DROP TABLE IF EXISTS review_decisions;
