...
describe("core SQLite migration", () => {
  it("creates the current core tables with foreign keys enabled and can roll back", () => {
    ...
    expect(database.prepare("SELECT version FROM schema_migrations ORDER BY version").all().map((row) => row["version"])).toEqual([1, 2, 3, 4, 5]);
  ...
  it("validates an applied migration without applying it", () => {
    const database = openDatabase(":memory:");
    expect(() => validateMigration(database)).toThrow("migration 5 is not applied");
    migrate(database);
