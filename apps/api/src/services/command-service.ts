...
export class CommandService {
  ...
  createRun(input: unknown, actor: PolicyActor, idempotencyKey: string, traceId: string): AcceptedCommand {
    ...
    return withTransaction(this.options.database, () => {
      ...
      this.queue.enqueue({ id: jobId, workspace_id: body.workspace_id, run_id: runId, step_id: stepId, idempotency_key: `queue:${idempotencyKey}`, request_hash: requestHash({ runId, stepId }), available_at: this.now(), max_attempts: 3, payload: { kind: "deterministic" } });
  ...
  retryRun(input: unknown, runId: string, actor: PolicyActor, idempotencyKey: string, expectedVersion: number, traceId: string): AcceptedCommand {
    ...
    return withTransaction(this.options.database, () => {
      ...
      this.queue.enqueue({ id: jobId, workspace_id: body.workspace_id, run_id: runId, step_id: stepId, idempotency_key: `queue:${idempotencyKey}`, request_hash: requestHash({ runId, stepId }), available_at: this.now(), max_attempts: 3, payload: { kind: "retry" } });
